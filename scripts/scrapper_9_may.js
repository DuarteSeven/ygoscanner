...
// Helper to convert "X days ago" into a real Date object for math
function parseRelativeDate(dateStr) {
    ...
    // Create a new date object based on 'now'
    const date = new Date();

    if (s.includes('hour')) date.setHours(date.getHours() - num);
    else if (s.includes('day')) date.setDate(date.getDate() - num);
    else if (s.includes('week')) date.setDate(date.getDate() - (num * 7));
    else if (s.includes('month')) date.setMonth(date.getMonth() - num);
    else if (s.includes('yesterday')) date.setDate(date.getDate() - 1);
    
    return date;
}

async function runScraper() {
    // CHANGE THIS IF NEEDED: The specific start date
    const cutoffDate = new Date("2026-05-09T00:00:00");
    console.log(`Target: Fetching all decks submitted since ${cutoffDate.toDateString()}...`);
    ...
    while (keepGoing) {
        ...
        if (!decks || decks.error || decks.length === 0) {
            console.log("Reached end of data.");
            break;
        }

        for (const deck of decks) {
            const deckDate = parseRelativeDate(deck.submit_date);

            // STOP LOGIC: If the deck date is before May 9th, kill the loop
            if (deckDate < cutoffDate) {
                console.log(`Found deck from "${deck.submit_date}" (${deckDate.toDateString()}). Stopping.`);
                keepGoing = false;
                break;
            }
    ...
    fs.writeFileSync('./meta_data_may9.json', JSON.stringify(database, null, 2));
    console.log(`✅ Success! Analyzed ${database.totalDecks} decks since May 9th.`);
}
