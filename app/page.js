...
export default function Home() {
  ...
  // Helper to change filters and reset page
  ...
  const renderGrid = (cardsMap, totalDecks) => {
    ...
    let filtered = Object.entries(cardsMap).filter(([name]) => {
      ...
      // 2. Deck Location Check
      if (fDeckType === 'Extra' && (!isMonster || !info.isExtra)) return false;
      if (fDeckType === 'Main' && (isMonster && info.isExtra)) return false;
    ...
    const Pagination = () => (
      <div className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-2 px-4 rounded-xl shadow-lg">
        <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => Math.max(1, p-1))} className="hover:text-blue-500 disabled:opacity-10">◀</button>
        ...
        <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(p => Math.min(totalPages, p+1))} className="hover:text-blue-500 disabled:opacity-10">▶</button>
      </div>
    );

    return (
      <div className="space-y-8">
        <div className="flex justify-between items-center"><span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Found {filtered.length} Cards</span><Pagination /></div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {paginated.map(([name, data]) => (
            <div key={name} className="bg-zinc-900 border border-zinc-800 p-2 rounded-lg hover:border-blue-500 transition shadow-2xl">
  ...
  const renderIndividualDeck = (deck) => {
    const Row = ({title, list}) => (
      <div className="mb-8">
        <h3 className="text-zinc-600 font-black text-xs uppercase tracking-[0.3em] mb-4 border-b border-zinc-800 pb-2">{title} — {list.length}</h3>
    ...
    return (
      <div className="bg-zinc-950 p-6 md:p-10 rounded-3xl border border-zinc-800 shadow-inner">
        <div className="text-center mb-8"><h2 className="text-3xl font-black text-blue-400 uppercase">{deck.player}</h2><p className="text-zinc-500 font-bold text-[10px] tracking-[0.2em]">{deck.place} • {deck.event}</p></div>
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-10 font-sans">
      <header className="flex flex-col items-center mb-10 text-center">
        <h1 className="text-6xl font-black tracking-tighter italic text-blue-600 mb-8 drop-shadow-[0_0_15px_rgba(37,99,235,0.3)] uppercase">Pro Meta Scanner</h1>
        <div className="flex bg-zinc-900 p-1 rounded-2xl border border-zinc-800 gap-1 flex-wrap justify-center">
          {['overall', 'side', 'archetypes'].map(t => (
            <button key={t} onClick={() => { setActiveTab(t); setSelectedDeckIndex("NONE"); resetFilters(); }} className={`px-10 py-3 rounded-xl font-black text-xs uppercase transition ${activeTab === t ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>{t}</button>
          ))}
        </div>
      </header>

      {/* --- RE-RESTORED FULL FILTER BAR --- */}
      {selectedDeckIndex === "NONE" && (
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl mb-12 shadow-2xl space-y-6">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="flex flex-col"><label className="text-[10px] text-zinc-500 uppercase font-bold mb-1">Category</label><select value={fMainType} onChange={(e) => handleFilterChange(setFMainType, e.target.value)} className="bg-black border border-zinc-700 rounded-lg p-2 text-xs outline-none"><option value="All">All Types</option><option value="Monster">Monster</option><option value="Spell">Spell</option><option value="Trap">Trap</option></select></div>
          ...
          <div className="space-y-10">
            <div className="flex flex-wrap gap-2 justify-center">
              {Object.entries(metaData.archetypes).sort((a,b) => b[1].count - a[1].count).map(([name, data]) => (
                <button key={name} onClick={() => { setSelectedArch(name); setSelectedDeckIndex("NONE"); }} className={`px-4 py-1.5 rounded-full text-[10px] font-black border transition ${selectedArch === name ? 'bg-blue-600 border-blue-400 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-500'}`}>{name} — {data.count}</button>
              ))}
            </div>
            {selectedArch && (
              <div className="space-y-8">
                <div className="flex flex-col items-center gap-6">
                  <h2 className="text-4xl font-black italic text-blue-500 uppercase">{selectedArch}</h2>
                  <div className="flex flex-col items-center bg-zinc-900 p-4 rounded-3xl border border-zinc-800 shadow-xl">
                    <label className="text-[10px] font-black text-zinc-600 uppercase mb-2">Topping Decklists</label>
                    <select value={selectedDeckIndex} onChange={(e) => setSelectedDeckIndex(e.target.value)} className="bg-black border border-zinc-700 text-blue-400 rounded-xl p-3 text-xs font-black outline-none w-80">
                      <option value="NONE">NONE (Play-Rate Statistics)</option>
                ...
                {selectedDeckIndex === "NONE" ? (
                  <div>
                    <div className="flex justify-center mb-8"><div className="flex bg-black p-1 rounded-xl text-[10px] font-bold uppercase border border-zinc-800"><button onClick={() => setArchSubTab('main')} className={`px-8 py-2 rounded-lg transition ${archSubTab === 'main' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500'}`}>Main Deck Stats</button><button onClick={() => setArchSubTab('side')} className={`px-8 py-2 rounded-lg transition ${archSubTab === 'side' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500'}`}>Side Deck Stats</button></div></div>
